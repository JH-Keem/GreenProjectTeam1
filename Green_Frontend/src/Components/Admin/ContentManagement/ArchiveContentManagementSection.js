...
const ArchiveContentManagementSection = () => {
  ...
  async function modifyData(modifyData){
    const formData = new FormData();
      ...
      formData.append('viewCount', modifyData.viewCount);
      formData.append('sectionNo', 2); // 섹션 번호 추가
  ...
  const modifyBoard = (archive, event) => {
    ...
    setViewCount(archive.viewCount);
    setModalTitle('수정하기');    
    setModalBtn('수정')
        ...
        <div className="container-fluid">
            ...
            <div className="col-12 mt-4">
                <ul className="pagination justify-content-center mb-0">
                    {page.prev && (
                      <li className="page-item"><a style={{ cursor: 'pointer' }} className="page-link" onClick={()=>{ page.prev && axiosPageData(page.startPage-1)}} aria-label="Previous">Prev</a></li>
                    )}
                        {Array.from({ length: (page.endPage - page.startPage + 1) }, (_, i) => page.startPage + i).map(pageNum => (
                        <li className={`page-item ${pageNum === page.pagingInfo.pageNum ? 'active' : ''}`} key={pageNum}><a style={{ cursor: 'pointer' }} className="page-link" onClick={() => {axiosPageData(pageNum)}}>{pageNum}</a></li>
                        ))}
                    {page.next && (
